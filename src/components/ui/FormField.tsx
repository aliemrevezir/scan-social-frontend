...
export function FormField({
  ...
  if (variant === 'inside') {
    const trailing = infoText ? <InfoIcon text={infoText} /> : undefined;
    const control = isValidElement(children)
      ? cloneElement(children as ReactElement<InputProps>, {
          'data-inside-label': true,
          trailing,
        })
      : children;
...
function InfoIcon({ text, className }: InfoIconProps) {
  return (
    <div className={clsx('group relative inline-flex', className)}>
      <button
        type="button"
        className="inline-flex h-[22px] w-[22px] items-center justify-center rounded-full text-[color:var(--color-muted-light)] transition-colors hover:text-[color:var(--color-primary)] focus:outline-none focus:ring-2 focus:ring-[color:var(--color-primary)]/25"
        aria-label="Field information"
      >
        <span className="material-symbols-outlined text-[17px] leading-none">info</span>
      </button>
      <div className="pointer-events-none absolute right-0 top-full z-50 hidden w-64 translate-y-2 rounded-2xl border border-[color:var(--color-border)] bg-white px-3 py-2 text-left text-xs text-[color:var(--color-text-secondary)] shadow-dropdown group-hover:flex group-focus-within:flex">
        {text}
      </div>
    </div>
  );
